...
Фазы 1–5 закрыты по roadmap (включая автозагрузку XML и отзывы).
Дальше — фаза 6 (вторая площадка, Ozon) или полировка/деплой.
Подробности в `docs/03-roadmap.md` бэкенд-репозитория.

Вебхук мессенджера и XML-фид требуют `PUBLIC_BASE_URL` с HTTPS на бэкенде.
